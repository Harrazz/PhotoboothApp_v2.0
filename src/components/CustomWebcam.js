    ...
    const downloadCollage = () => {
        ...
        htmlToImage.toJpeg(photoGrid)
            .then(function (dataUrl) {
                const a = document.createElement('a');
                a.href = dataUrl;
                a.download = 'family-day-gfis(collage).jpeg';
                a.click();
            });
    };
        ...
        <div className="container-box">
            ...
            <div className="container" >
                <div className={`navbar ${capturedPhotos.length > 0 ? "hidden" : ""}`}>
                    {!imgSrc && (
                        <>
                            <button onClick={handleColumnsClick} className="circle"><i className="bi bi-columns"></i></button>
                            {showCollage && (
                                <>
                                    <button
                                        onClick={() => selectGrid(4)}
                                        className={selectedButton === 4 ? "selected" : "circle"}>
                                        <i className="bi bi-4-square-fill"></i>
                                    </button>
                                    <button
                                        onClick={() => selectGrid(6)}
                                        className={selectedButton === 6 ? "selected" : "circle"}>
                                        <i className="bi bi-6-square-fill"></i>
                                    </button>
                                </>
                            )}
                            <button onClick={toggleFilterButtons} className="circle"><i className="bi bi-magic"></i></button>
                            <button onClick={toggleMirror} className="circle"><i className="bi bi-symmetry-vertical"></i></button>
                            <button onClick={toggleCameraMode} className="circle"><i className="bi bi-arrow-repeat"></i></button>
                        </>
                    )}
                </div>
                ...
                {selectedButton === 4 || selectedButton === 6 ? null : (
                    <div className="btn">
                        {countdownActive ? ( // Disable the button during countdown
                            ...
                            imgSrc ? (
                                <>
                                    <button onClick={retake} className="circle"><i className="bi bi-arrow-clockwise"></i></button>
                                    <button onClick={downloadSingle} className="circle"><i className="bi bi-floppy2"></i></button>
                                </>
                            ) : (
                                <>
                                    <button onClick={() => startCountdown(3)} className="circle"><i className="bi bi-3-circle-fill"></i></button>
                                    <button onClick={capture} className="circle shutter"><i className="bi bi-camera-fill"></i></button>
                                    <button onClick={() => startCountdown1(5)} className="circle"><i className="bi bi-5-circle-fill"></i></button>
                                </>
                            )
                        )}
                    </div>
                ...
                <div className="btn">
                    {capturedPhotos.length < selectedGrid && (selectedButton === 4 || selectedButton === 6) &&
                        (<>
                            <button onClick={collage} disabled={!cameraEnabled} className="circle shutter"
                    ...
                    {capturedPhotos.length === selectedGrid && (
                        <>
                            {(selectedButton === 4 || selectedButton === 6) && (
                                <>
                                    <button onClick={retake} className="circle"><i className="bi bi-arrow-clockwise"></i></button>
                                    <button onClick={downloadCollage} className="circle"><i className="bi bi-floppy2"></i> </button>
